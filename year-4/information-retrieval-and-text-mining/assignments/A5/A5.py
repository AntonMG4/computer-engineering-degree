...
class SimpleScorer(Scorer):
    def score_term(self, term: str, query_freq: int) -> None:
        if term in self.index[self.field]:
            # Retrieve the document frequencies for the term in the field
            doc_freqs = self.index[self.field][term]  
            for doc_id, doc_freq in doc_freqs:
                self.scores[doc_id] += doc_freq * query_freq
...
class ScorerBM25(Scorer):
    ...
    def score_term(self, term: str, query_freq: int) -> None:
        ...
        doc_freqs = self.index[self.field].get(term, [])  
        n_t = len(doc_freqs)  

        idf_t = math.log(N/n_t)
        avgdl = self.collection.avg_field_length(self.field) 
         
        for doc_id, c_t_d in doc_freqs:
            doc_len = len(self.collection[doc_id][self.field])  
            
            # Calculate the normalization factor B
            B = 1 - self.b + self.b * (doc_len / avgdl)
            
            # Compute the BM25 score for the term in the current document
            score = ((c_t_d * (self.k1 + 1)) / (c_t_d + self.k1 * B)) * idf_t
